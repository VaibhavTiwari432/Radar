...
| J6 | An agile radar (per-frame sweep reversal) counters the swarm | **NO against a true repeater; it BACKFIRES against a stale one [SIM, 12 Sep 2026].** Fresh: every cell within one seed of agility-off (A1). Stale, where the repeaters replay last frame's chirp: phantoms lose detections (real 40→26, 80→52), but the skin backtrack catches **0** in every cell (was 40/40, 80/80) and emitter attribution falls 40→10. Traced: stale tracks hold 1–3 usable azimuths, the counters need ≥ 4 and abstain, and the label still passes them. A judge-side gap, not a physical escape. Agility alone also costs 2/40 real on genuine targets (untraced). A3/A4 falsified, disclosed | `SWARM_RESULTS.md` Phase 10, predictions A1–A4 |
